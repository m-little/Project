...
exports.search_results = function(req, res)
{
	...
	if (req.query.t == 'w') {
		...
		dao.query("select r.recipe_id, r.recipe_name, r.description, p.picture_id, p.caption, p.location from recipe r JOIN recipe_picture rp, picture p where MATCH(r.recipe_name) AGAINST(\"" + dao.safen(req.query.q) + "\") AND r.public = 1 AND rp.recipe_id = r.recipe_id AND p.picture_id=rp.picture_id GROUP BY r.recipe_id;", output2);
