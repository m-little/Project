exports.Recipe = function Recipe(id_, owner_, public_, name_, cate_, serv_, prep_, ready_, dir_, date_added_, date_edited_)
{
	...
	this.ingredients_string = function()
	{
		...
		for (var i = 0; i < this.ingredients.length; i++)
			ingredients.push("{name: '" + this.ingredients[i].name.replace("'", "\\'") + "', picture_location: '" + this.ingredients[i].picture.location + "', id: '" + this.ingredients[i].id + "', use_count: " + this.ingredients[i].use_count + "}");
		return ingredients;
	}
